...
const MOCK_REPORTS: Report[] = [
  {
    ...
    currentPrice: 18.42,
    targetPrice: 31.0,
    executiveSummary: "Sole qualified vendor for single-wafer wet-clean tools at CXMT. Import-substitution tailwinds drive >68% upside to our $31 price target.",
    featured: true,
  },
  {
    slug: "cvx-may2025",
    ...
    currentPrice: 152.3,
    targetPrice: 178.0,
    executiveSummary: "Permian Basin optionality and Hess acquisition synergies underappreciated by the market. FCF yield of 8.2% supports continued buyback acceleration.",
    featured: true,
  },
  {
    slug: "uri-may2025",
    ...
    currentPrice: 618.4,
    targetPrice: 740.0,
    executiveSummary: "Infrastructure bill tailwinds and specialty rental mix shift drive durable earnings power. Management guidance conservative relative to our channel checks.",
  },
  {
    slug: "intc-apr2025",
    ticker: "INTC",
    ...
    currentPrice: 21.8,
    targetPrice: 24.0,
    executiveSummary: "18A process node remains a wildcard. Foundry losses narrowing but timeline to profitability still uncertain. Await clearer execution signal.",
  },
  {
    slug: "xom-apr2025",
    ticker: "XOM",
    ...
    currentPrice: 108.5,
    targetPrice: 112.0,
    executiveSummary: "Pioneer integration on track but limited re-rating catalyst near-term. Dividend safety strong; prefer CVX for upside.",
  },
];
...
const ALL_SECTORS = ["All", ...Array.from(new Set(MOCK_REPORTS.map((r) => r.sector)))];
const ALL_RATINGS: ("All" | Rating)[] = ["All", "Strong Buy", "Buy", "Hold", "Sell", "Strong Sell"];
    ...
    >
      ...
      >
        <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 16 }}>
          ...
          <span style={{ marginLeft: "auto", fontSize: 11, color: "#555" }}>{report.sector}</span>
        </div>

        <h3 style={{ fontSize: 20, fontWeight: 800, color: "#fff", margin: "0 0 10px", lineHeight: 1.25, letterSpacing: "-0.02em" }}>
          {report.companyName}
        </h3>
        ...
        <div style={{ display: "flex", gap: 20, borderTop: "1px solid #1E2530", paddingTop: 16 }}>
          <div>
            <div style={{ fontSize: 10, color: "#555", textTransform: "uppercase", letterSpacing: "0.06em", marginBottom: 3 }}>Price</div>
            <div style={{ fontSize: 15, fontWeight: 700, color: "#ccc" }}>${report.currentPrice.toFixed(2)}</div>
          </div>
          <div>
            <div style={{ fontSize: 10, color: "#555", textTransform: "uppercase", letterSpacing: "0.06em", marginBottom: 3 }}>Target</div>
            <div style={{ fontSize: 15, fontWeight: 700, color: "#4A9EFF" }}>${report.targetPrice.toFixed(2)}</div>
          </div>
          <div>
            <div style={{ fontSize: 10, color: "#555", textTransform: "uppercase", letterSpacing: "0.06em", marginBottom: 3 }}>Upside</div>
            <div style={{ fontSize: 15, fontWeight: 700, color: pos ? "#6DBF40" : "#E24B4A" }}>
              {pos ? "+" : ""}{upside}%
            </div>
          </div>
          <div style={{ marginLeft: "auto", textAlign: "right" }}>
            <div style={{ fontSize: 10, color: "#555", textTransform: "uppercase", letterSpacing: "0.06em", marginBottom: 3 }}>Published</div>
            <div style={{ fontSize: 12, color: "#666" }}>{report.publishedAt}</div>
          </div>
        </div>
      </div>
    </a>
    ...
    >
      ...
      >
        <span style={{
          ...
          background: "#111", color: "#fff", padding: "2px 7px", borderRadius: 4,
          display: "inline-block",
        }}>
          {report.ticker}
        </span>
        <div>
          <div style={{ fontSize: 14, fontWeight: 600, color: "#111", marginBottom: 2 }}>{report.companyName}</div>
          <div style={{ fontSize: 12, color: "#999" }}>{report.sector} · {report.analyst}</div>
        </div>
        <RatingBadge rating={report.rating} />
        <span style={{ fontSize: 14, color: "#555" }}>${report.currentPrice.toFixed(2)}</span>
        <span style={{ fontSize: 14, fontWeight: 600, color: "#185FA5" }}>${report.targetPrice.toFixed(2)}</span>
        <UpsideBadge current={report.currentPrice} target={report.targetPrice} />
      </div>
    </a>
  );
}
...
export default function HomePage() {
  ...
  return (
    <div style={{ minHeight: "100vh", background: "#F7F7F5", fontFamily: "'Inter', 'Helvetica Neue', sans-serif" }}>
      ...
      {/* Navbar */}
      <nav style={{
        background: "#0D1117", padding: "0 40px",
        ...
        <a href="/" style={{ color: "#fff", fontSize: 13, textDecoration: "none", fontWeight: 500 }}>Research</a>
        {/*<a href="/about" style={{ color: "#666", fontSize: 13, textDecoration: "none" }}>About</a>      In case we want about*/} 
      ...
      <div style={{ background: "#0D1117", padding: "52px 40px 48px" }}>
        <div style={{ maxWidth: 1100, margin: "0 auto" }}>
          ...
          <h1 style={{
            ...
            Texas Valuation<br />& Modeling
          </h1>
          <p style={{ fontSize: 16, color: "#666", margin: "0 0 36px", maxWidth: 480, lineHeight: 1.6 }}>
            Conviction-based equity research. Long-term theses, transparent process, institutional-quality analysis.
          </p>
          ...
          <div style={{ display: "flex", gap: 40 }}>
            {[
              ...
              { label: "Sectors covered", value: `${new Set(MOCK_REPORTS.map(r => r.sector)).size}` },
              { label: "Strong Buy ideas", value: `${MOCK_REPORTS.filter(r => r.rating === "Strong Buy").length}` },
            ].map(({ label, value }) => (
              <div key={label}>
                <div style={{ fontSize: 28, fontWeight: 800, color: "#fff", letterSpacing: "-0.02em" }}>{value}</div>
        ...
        {featured.length > 0 && (
          <section style={{ marginBottom: 56 }}>
            <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 18 }}>
              ...
              <span style={{ fontSize: 12, color: "#aaa" }}>{featured.length} ideas</span>
            </div>
            <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(320px, 1fr))", gap: 16 }}>
              {featured.map((r) => <FeaturedCard key={r.slug} report={r} />)}
            </div>
          </section>
        )}
      ...
      <footer style={{
        ...
        <p style={{ fontSize: 11, color: "#333", margin: 0, lineHeight: 1.6 }}>
          Research published for informational purposes only. Not investment advice. Past performance is not indicative of future results.
        </p>
      </footer>
    </div>
  );
